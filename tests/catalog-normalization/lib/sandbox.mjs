// sandbox.mjs — harness de CATALOG NORMALIZATION V1 (base 976bc93).
// Copia adaptada del patrón fix02/03/04: carga los scripts REALES del producto en
// node:vm con el mismo orden de index.html relevante para guardarProd/_naNormalizeData
// (utils, inline-01..07 con el wrapper F10 de inline-07, inline-10..12, inline-16).
// inline-08 (emoji) se omite como en fix04: no altera la operación bajo prueba.
// El stub de document.addEventListener es no-op: la instalación del normalizador se
// invoca directamente vía window._naInstallCatalogNameNorm() (en el navegador real la
// dispara DOMContentLoaded, registrado por inline-01 antes que el arranque de inline-03).
...
export function createPosSandbox(opts = {}) {
  ...
  const sandbox = {
    ...
    navigator: { userAgent: 'POS-CATALOG-NORM-Harness/1.0' },
  ...
  `, ctx, { filename: 'catalog-norm-bootstrap.js' });

  return {
    ctx,
    stores: { localStorage, sessionStorage },
    run(code) {
      return vm.runInContext(code, ctx, { filename: 'catalog-norm-harness.js' });
    },
    el(id) {
