...
test('startup persistence generated by loadAllData cannot create a false remote pending operation', () => {
  const boot = loadAllDataOverride();
  assert.match(boot, /var previousSuppressRemoteSave = state\.suppressRemoteSave/);
  assert.match(boot, /state\.suppressRemoteSave = true/);
  assert.match(boot, /finally \{\s*state\.suppressRemoteSave = previousSuppressRemoteSave;\s*\}/);
});

test('revision conflict exposes an explicit Use D1 LAB recovery path', () => {
  assert.match(source, /id="naLabUseRemote"/);
  assert.match(source, /Usar D1 LAB/);
