...
export async function handleA6(request, url, env, helpers) {
  ...
  if (provenance) {
    if (request.method !== 'GET') return helpers.json({ error:'method_not_allowed' },405,{ allow:'GET, OPTIONS' });
    const auth = await helpers.authorizeDevice(request.headers.get('x-device-id'),request,env,true); if (auth instanceof Response) return auth;
  ...
  const auth=await helpers.authorizeDevice(deviceId,request,env,true); if (auth instanceof Response) return auth;
