...
function lastCommitIndexForPath(commits, target) {
  let found = -1;
  for (let i = 0; i < commits.length; i += 1) {
    if (changedInCommit(commits[i]).includes(target)) found = i;
  }
  return found;
}
...
function verifyTemporalOrder(baseRef, taskPath, receiptPath) {
  ...
  const taskIndex = lastCommitIndexForPath(commits, taskPath);
  const receiptIndex = lastCommitIndexForPath(commits, receiptPath);
  const errors = [];
  ...
  if (taskIndex >= firstImplementation) errors.push('la última modificación del Task Contract debe ocurrir antes del primer cambio funcional');
  if (receiptIndex >= firstImplementation) errors.push('la última modificación del recibo SKILL_PREFLIGHT debe ocurrir antes del primer cambio funcional');
  return errors;
}
