...
isModuleLocked=function(moduleName,options){if(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()&&!(moduleName==='ventas'&&options?.canonicalSaleCapture===true))return true;if(securityIsLocked())return true;const master=storage.getItem(LOCK_KEYS.master)==='true',readOnly=storage.getItem(LOCK_KEYS.readOnly)==='true';if(master||readOnly)return true;const key=LOCK_KEYS.modules[moduleName];return key?storage.getItem(key)==='true':false;};
...
cliRender=function(){
  if(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()&&_naCanonicalLoadError){
    ...
    const wrap=document.getElementById('cliList');if(wrap){wrap.replaceChildren();const msg=document.createElement('div');msg.className='empty-state';msg.textContent='Canónico no disponible. Comprueba tu conexión y vuelve a intentar.';const retry=document.createElement('button');retry.type='button';retry.textContent='Reintentar conexión';retry.style.cssText='display:block;margin:12px auto;padding:10px 16px;min-height:44px;border:0;border-radius:8px;background:#0f766e;color:#fff;font-weight:700;cursor:pointer';retry.addEventListener('click',async()=>{retry.disabled=true;retry.textContent='Conectando…';try{await NuevoAmanecerCanonical.startPOS();const canonical=NuevoAmanecerCanonical.legacySnapshot();productos=canonical.products;clientes=canonical.customers;creditos=canonical.credits;_naCanonicalLoadError=null;posRender();posUpdateCart();invRender();cfgUpdateStats();updateDashboard();const meta=NuevoAmanecerCanonical.snapshot();if(badge){badge.hidden=false;badge.textContent='Canónico | '+String(meta.customers.length)+' clientes';badge.style.cssText='padding:5px 9px;border-radius:999px;background:#ccfbf1;color:#115e59;font-size:11px;font-weight:800';}const status=document.querySelector('#localStatus span'),save=document.getElementById('saveStatus');if(status)status.textContent='Autoridad canónica de solo lectura';if(save)save.textContent='Solo lectura | sin escrituras';cliRender();}catch(error){_naCanonicalLoadError=error;productos=[];clientes=[];creditos=[];posRender();invRender();const status=document.querySelector('#localStatus span'),save=document.getElementById('saveStatus');if(status)status.textContent='Canónico no disponible';if(save)save.textContent='Canónico no disponible';cliRender();}});msg.append(document.createElement('br'),retry);wrap.append(msg);}return;
  }
  _naLocalCliRender();
...
window.addEventListener('offline',()=>{if(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()){const state=NuevoAmanecerCanonical.sourceState(),status=document.querySelector('#localStatus span'),badge=document.getElementById('cliAuthorityBadge');if(state.source==='cache'){state.validation='offline';if(status)status.textContent='Local sincronizado anteriormente / sin conexi\u00f3n';if(badge){badge.hidden=false;badge.textContent='Cache can\u00f3nico | sin conexi\u00f3n';}}else{_naCanonicalLoadError=new Error('AUTHORITY_UNAVAILABLE');cliRender();if(status)status.textContent='Can\u00f3nico no disponible';}}});
window.addEventListener('na:canonical-updated',()=>{if(typeof NuevoAmanecerCanonical==='undefined'||!NuevoAmanecerCanonical.enabled())return;const canonical=NuevoAmanecerCanonical.legacySnapshot(),state=NuevoAmanecerCanonical.sourceState();productos=canonical.products;clientes=canonical.customers;creditos=canonical.credits;_naCanonicalLoadError=null;posRender();posUpdateCart();invRender();cfgUpdateStats();updateDashboard();cliRender();const status=document.querySelector('#localStatus span'),badge=document.getElementById('cliAuthorityBadge');if(status)status.textContent=state.validation==='offline'?'Local sincronizado anteriormente / sin conexi\u00f3n':state.validation==='stale'?'Cache can\u00f3nico | reintentar actualizaci\u00f3n':'Autoridad can\u00f3nica validada';if(badge){badge.hidden=false;badge.textContent=state.validation==='offline'?'Cache can\u00f3nico | sin conexi\u00f3n':'Can\u00f3nico | '+String(canonical.customers.length)+' clientes';}});
window.addEventListener('storage',event=>{if(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()&&(!event.key||event.key==='na_canonical_binding')){_naCanonicalLoadError=new Error('STALE_AUTHORITY_BINDING');cliRender();const status=document.querySelector('#localStatus span');if(status)status.textContent='Can?nico no disponible';}});
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()){
    try{await NuevoAmanecerCanonical.startPOS();const canonical=NuevoAmanecerCanonical.legacySnapshot();productos=canonical.products;clientes=canonical.customers;creditos=canonical.credits;}
    catch(error){_naCanonicalLoadError=error;productos=[];clientes=[];creditos=[];}
  }
  loadAppState();loadMasterConfig();_naInitSecurity();_naNormalizeData();renderCategorySelects();_naApplyConfigUI();_naInitFreeSaleShortcut();_naInitBarcodeScanner();creditos.forEach(_naSyncCreditStatus);posRender();posUpdateCart();invRender();cfgUpdateStats();updateDashboard();
  const cliBadge=document.getElementById('cliAuthorityBadge');if(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()&&!_naCanonicalLoadError&&cliBadge){const meta=NuevoAmanecerCanonical.snapshot();cliBadge.hidden=false;cliBadge.textContent='Can\u00f3nico | '+String(meta.customers.length)+' clientes';cliBadge.style.cssText='padding:5px 9px;border-radius:999px;background:#ccfbf1;color:#115e59;font-size:11px;font-weight:800';}
  cliRender();
  const localStatus=document.querySelector('#localStatus span'),saveStatus=document.getElementById('saveStatus');if(localStatus&&typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()){const state=NuevoAmanecerCanonical.sourceState();localStatus.textContent=_naCanonicalLoadError?'Can\u00f3nico no disponible':state.source==='cache'?'Cache local | validando actualizaci\u00f3n':'Autoridad can\u00f3nica validada';if(saveStatus)saveStatus.textContent=_naCanonicalLoadError?'Can\u00f3nico no disponible':'Solo lectura | sin escrituras';const badge=document.getElementById('cliAuthorityBadge');if(badge&&!_naCanonicalLoadError&&state.source==='cache'){badge.hidden=false;badge.textContent='Cache can\u00f3nico | validando';}}
  ...
  await saveAllData();
  if(typeof NuevoAmanecerOutbox!=='undefined'&&!(typeof NuevoAmanecerCanonical!=='undefined'&&NuevoAmanecerCanonical.enabled()))NuevoAmanecerOutbox.start(_naQueueCloudSyncPersist);
});
